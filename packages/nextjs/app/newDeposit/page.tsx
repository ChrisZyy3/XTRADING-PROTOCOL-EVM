...
const DepositSection = ({ balance }: { balance: string }) => {
    ...
    const handleApprove = async () => {
        ...
        const amountToApprove = depositAmount ? parseUnits(depositAmount, decimals) : BigInt("115792089237316195423570985008687907853269984665640564039457584007913129639935");
    ...
    const needsApproval = (allowance || 0n) === 0n || (!!depositAmount && (allowance || 0n) < parseUnits(depositAmount, tokenDecimals || 18));
    ...
    return (
        <div className="card card-premium p-8 h-full">
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={refreshAll} className="btn btn-ghost btn-xs text-gray-400 hover:text-white" title="Refresh all balances">
                    <ArrowPathIcon className="w-4 h-4" />
                </button>
            </div>

            <p className="text-gray-400 text-sm mb-6">
                {t.newDeposit.description}
            </p>
            ...
            <div className="bg-[#1a2c25] rounded-xl p-4 mb-6 border border-[#203731] text-sm space-y-2 font-mono">
                <div className="flex justify-between">
                    ...
                    <span className="text-white truncate max-w-[150px]" title={address}>{address}</span>
                </div>
                <div className="flex justify-between">
                    <span className="text-gray-500">{t.newDeposit.bnb}:</span>
                    <span className="text-[#F0B90B]">{bnbBalance ? parseFloat(bnbBalance?.formatted).toFixed(4) : "..."} BNB</span>
                </div>
                <div className="flex justify-between">
                    <span className="text-gray-500">{t.newDeposit.tcmBalance}:</span>
                    <span className="text-[#39FF14]">{tokenBalance ? parseFloat(formatUnits(tokenBalance, tokenDecimals || 18)).toFixed(2) : "..."} TCM</span>
                </div>
                <div className="flex justify-between">
                    <span className="text-gray-500">{t.newDeposit.vaultBalance}:</span>
                    <span className="text-blue-400">{vaultBalance ? parseFloat(formatUnits(vaultBalance, tokenDecimals || 18)).toFixed(2) : "..."} TCM</span>
                </div>
                <div className="flex justify-between">
            ...
            <div className="space-y-4">
                ...
                <div className="flex gap-2 mt-4">
                    ...
                    <button
                        className={`btn btn-warning flex-1 font-bold ${needsApproval ? "btn-outline opacity-50" : ""}`}
            ...
            <div className="mt-8 text-xs text-gray-500">
                <p><strong>{t.newDeposit.note.title}</strong></p>
