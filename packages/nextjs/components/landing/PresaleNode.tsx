...
import { useBuyNode, useNodeStock, useNodeTypes } from "~~/hooks/api/useNodes";
...
export const PresaleNode = () => {
  ...
  const { data: nodeStock, isLoading: isStockLoading } = useNodeStock();
        ...
        <div className="grid gap-8">
          {isLoading ? (
            ...
            nodeTypes?.data?.map((item: any, index: number) => {
              const stock = nodeStock?.data?.by_type?.find(entry => entry.type === item.type);
                ...
                >
                  ...
                  <div className="px-6 py-6">
                    <div className="grid grid-cols-4 text-sm md:text-base text-center text-white">
                      ...
                      <span>{item.hash_power || item.hashpower}</span>
                      <span>{item.tcm_locked || "-"}</span>
                      <span>{item.usd_amount || item.price || "0"}</span>
                    </div>
                    ...
                    <div className="mt-6 flex items-center justify-between gap-6 bg-[#0A1813] p-2 rounded-xl">
                      <div className="flex-1 flex flex-col gap-2 min-w-0">
                        <div className="flex items-center justify-between text-xs text-gray-500 tracking-widest uppercase">
                          ...
                          <span className="text-gray-400 tracking-normal">
                            {isStockLoading ? t.presale.loading : `${sold}/${total}`}
                          </span>
                        </div>
          ...
          <div className="w-full max-w-sm bg-[#0A1813] border border-[#203731] rounded-2xl p-6 relative shadow-[#39FF14]/10 shadow-2xl">
            ...
            <div className="text-center text-sm text-gray-300">
              ...
              <div className="mt-2 text-[#27E903] font-semibold uppercase">
                {(() => {
                  const node = nodeTypes?.data?.find((item: any) => item.type === confirmNodeType);
                  if (!node) return confirmNodeType;
                  return language === "zh" ? node.name : node.type;
                })()}
              </div>
            </div>
